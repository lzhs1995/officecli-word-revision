...
ZoteroRefresh is an asynchronous launcher. On macOS, a stale or inconsistent per-user TCC database can make the native Zotero route appear dead even when Word and Zotero have Full Disk Access; see [macos-tcc-zotero-word.md](macos-tcc-zotero-word.md) for evidence-first recovery and the backed-up last-resort reset. The owned-document helper waits for a document update, and the adapter verifies citation identities and non-citation content. A changed author-year suffix can be valid CSL disambiguation and must be logged. An unexpected modal is a failure requiring inspection, not proof that Refresh succeeded. Do not close unrelated documents or terminate the user's Word application to recover one job.
